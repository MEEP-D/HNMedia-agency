(function () {
  ...
  function section(title, body) { return '<section class="py-8 reveal"><div class="max-w-6xl mx-auto px-4"><div class="flex items-center gap-2 mb-4"><span class="w-2 h-2 rounded-full bg-emerald-500"></span><h2 class="text-sm md:text-base font-medium text-slate-800 m-0">' + title + '</h2></div>' + body + '</div></section>'; }
  ...
  function renderSection(s) { var t = s.type || 'grid'; if (t === 'grid') { var g = '<div class="' + gridResponsive() + '">' + ((s.items || []).map(serviceCard).join('')) + '</div>'; var head = TF(s, 'title') || TR('Danh sách', 'List'); var sub = TF(s, 'subtitle') ? ('<p class="text-xs text-slate-600 mb-2">' + TF(s, 'subtitle') + '</p>') : ''; return section(head, sub + g); } if (t === 'gallery') { var imgs = (s.images || []).map(function (it) { if (typeof it === 'string') { return it; } if (it && typeof it === 'object') { return it.image || it.url || ''; } return ''; }).filter(function (x) { return !!x; }); var g = '<div class="' + galleryGrid() + '">' + imgs.map(function (src) { return imageTile(src, TF(s, 'title') || ''); }).join('') + '</div>'; return section(TF(s, 'title') || TR('Bộ sưu tập', 'Gallery'), g); } if (t === 'text') { var body = (lang() === 'en' && s.body_en ? s.body_en : (s.body || '')); var b = '<div class="' + cardGlass() + ' p-4"><div class="text-sm text-slate-700">' + body + '</div></div>'; return section(TF(s, 'title') || TR('Thông tin', 'Info'), b); } if (t === 'cta') { var a = '<a class="inline-flex items-center gap-2 rounded-xl bg-emerald-600 text-white text-sm px-4 py-2 hover:bg-emerald-600/90 shadow-sm" href="' + (s.ctaLink || '#') + '"><i data-lucide="arrow-right" class="h-4 w-4"></i><span>' + (TF(s, 'ctaText') || TR('Xem thêm', 'Learn more')) + '</span></a>'; return section(TF(s, 'title') || TR('Hành động', 'Action'), a); } return ''; }
  function partnerLogo(p) { var src = (typeof p === 'string') ? p : (p && (p.image || p.logo || p.url)) || ''; var alt = (p && (p.name || p.title)) || TR('Đối tác', 'Partner'); return '<div class="shrink-0 h-10 w-auto inline-flex items-center"><img src="' + src + '" alt="' + alt + '" class="h-10 w-auto object-contain" loading="lazy" decoding="async"></div>'; }
  function renderPartners(home) { var list = (home && home.partners) || []; if (!list.length) return ''; var dur = Math.max(12, list.length * 3); var track = '<div class="flex items-center gap-8 marquee-track">' + list.concat(list).map(partnerLogo).join('') + '</div>'; var row = '<div class="overflow-hidden" data-marquee="true" data-marquee-duration="' + dur + '">' + track + '</div>'; var row2 = '<div class="overflow-hidden mt-3" data-marquee="true" data-marquee-duration="' + (dur + 4) + '">' + track + '</div>'; return section(TR('Đối tác', 'Partners'), row + row2); }
  ...
  function renderFooter(cfg, contact) { var f = document.getElementById('footer'); if (!f) return; var y = new Date().getFullYear(); var n = (cfg && cfg.brand && cfg.brand.name) || 'Media Agency'; var logo = (cfg && cfg.brand && cfg.brand.logo) || ''; var desc = (cfg && cfg.brand && (cfg.brand.description || cfg.brand.tagline)) || ''; var addr = (contact && contact.address) || ''; var phone = (contact && contact.phone) || ''; var email = (contact && contact.email) || ''; var left = logo ? ('<div class="flex items-center gap-2"><img src="' + logo + '" alt="' + n + '" class="h-6 w-auto rounded" decoding="async"><span class="text-sm font-medium text-slate-800">' + n + '</span></div>') : ('<div class="text-sm font-medium text-slate-800">' + n + '</div>'); var right = '<div class="text-xs text-slate-600"><div>' + (desc || '') + '</div>' + (addr ? ('<div class="mt-1">' + TR('Địa chỉ', 'Address') + ': ' + addr + '</div>') : '') + (phone ? ('<div>' + TR('Điện thoại', 'Phone') + ': ' + phone + '</div>') : '') + (email ? ('<div>Email: ' + email + '</div>') : '') + '<div class="mt-2">' + n + ' © ' + y + '</div></div>'; f.innerHTML = '<div class="grid grid-cols-1 md:grid-cols-2 gap-3 items-start">' + left + right + '</div>'; }
  function renderFAB(contact) { var fb = contact && contact.facebook; var zalo = contact && contact.zalo; var phone = (contact && (contact.hotline || contact.phone)); var old = document.getElementById('fab-social'); if (old && old.parentNode) { old.parentNode.removeChild(old); } if (!fb && !zalo && !phone) { return; } var wrap = document.createElement('div'); wrap.id = 'fab-social'; wrap.className = 'fixed bottom-4 right-4 z-50 flex flex-col gap-2'; var html = ''; if (phone) { html += '<a href="tel:' + phone + '" class="h-11 w-11 rounded-full shadow-lg border border-emerald-200 bg-emerald-600 text-white inline-flex items-center justify-center"><i data-lucide="phone" class="h-5 w-5"></i></a>'; } if (zalo) { html += '<a href="' + zalo + '" target="_blank" rel="noopener" class="h-11 w-11 rounded-full shadow-lg border border-sky-200 bg-sky-500 text-white inline-flex items-center justify-center"><i data-lucide="message-circle" class="h-5 w-5"></i></a>'; } if (fb) { html += '<a href="' + fb + '" target="_blank" rel="noopener" class="h-11 w-11 rounded-full shadow-lg border border-indigo-200 bg-indigo-600 text-white inline-flex items-center justify-center"><i data-lucide="globe" class="h-5 w-5"></i></a>'; } wrap.innerHTML = html; document.body.appendChild(wrap); if (window.lucide && typeof window.lucide.createIcons === 'function') { window.lucide.createIcons(); } }
  function renderHome(el, home, services, courses, portfolio) {
    var heroImg = (home.hero && home.hero.image) || 'images/hero/placeholder.jpg';
    var useVideo = !!(home.hero && home.hero.video);
    var useGif = !!(home.hero && home.hero.gif);
    var fullscreen = !!(home.hero && home.hero.fullscreen);
    var hero;
    if ((useVideo || useGif) && fullscreen) {
      var media = useVideo ? ('<video src="' + home.hero.video + '" class="absolute inset-0 w-full h-full object-cover" autoplay muted playsinline loop></video>') : ('<img src="' + home.hero.gif + '" class="absolute inset-0 w-full h-full object-cover" alt="Hero" loading="eager" decoding="async">');
    ...
    el.innerHTML = hero + '<section class="py-6"><div class="max-w-6xl mx-auto px-4">' + chips + '</div></section>' + intro + svc + renderPartners(home) + crs + cas + cta + renderSections(home.sections || []);
  }
  ...
  window.initContent = async function () { await load(); if (window.lucide && typeof window.lucide.createIcons === 'function') { window.lucide.createIcons(); } };
})();
