(function(){
  ...
  function section(title, body, style, animationAttr){ 
    ...
    return `<section class="py-12 reveal relative" style="${style}" ${aosAttr}>
              <div class="max-w-6xl mx-auto px-4 relative z-10">
                ${title ? `<div class="flex items-center justify-center mb-10" data-aos="fade-down"><h2 class="text-xl md:text-2xl font-bold text-slate-800 m-0 text-center uppercase tracking-wide border-b-2 border-emerald-500 pb-2">${title}</h2></div>` : ''}
                ${body}
              </div>
            </section>`; 
  }
  ...
  function cardBase(idx){ 
    return `data-aos="fade-up" data-aos-delay="${(idx||0)*100}" class="group h-full flex flex-col bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-xl hover:border-emerald-500/30 hover:-translate-y-1 transition-all duration-300 ease-out overflow-hidden"`; 
  }
    ...
    return `<div ${cardBase(i)}>
              ...
              <div class="p-5 flex flex-col flex-1">
                ...
                <div class="pt-4 border-t border-slate-100 flex items-center justify-between text-emerald-600 font-medium text-xs uppercase tracking-wide">
                  <span>Xem chi tiết</span> <i data-lucide="arrow-right" class="w-4 h-4"></i>
                </div>
              </div>
            </div>`; 
  }
  ...
  function newsCard(n, i){ 
    ...
    return `<div ${cardBase(i)}>
              ...
              <div class="p-5 flex flex-col flex-1">
                ...
                <div class="text-slate-400 text-xs font-bold uppercase tracking-wider group-hover:text-emerald-600 transition-colors">Đọc tiếp &rarr;</div>
              </div>
            </div>`; 
  }
    ...
    return `<div ${cardBase(i)}>
              <div class="p-6 flex flex-col h-full">
                <div class="flex justify-between items-start mb-4">
                  <div class="p-2 bg-emerald-50 text-emerald-600 rounded-lg"><i data-lucide="briefcase" class="w-6 h-6"></i></div>
                  <span class="bg-slate-100 text-slate-600 text-[10px] font-bold px-2 py-1 rounded uppercase">${TF(p,'location')}</span>
                </div>
                ...
                <button type="button" onclick="window.openApplyModal('${jobTitle}')" class="w-full inline-flex items-center justify-center gap-2 rounded-xl bg-slate-900 text-white text-sm font-bold px-4 py-3 hover:bg-emerald-600 transition-all shadow-md cursor-pointer">
  ...
  function personCard(t, i){ 
    return `<div ${cardBase(i)}>
              <div class="p-6 flex items-center gap-5">
                <img src="${t.photo||''}" alt="${TF(t,'name')}" class="w-16 h-16 rounded-full border-2 border-white shadow-md object-cover" loading="lazy">
                <div>
                  <div class="text-base font-bold text-slate-900">${TF(t,'name')}</div>
                  <div class="text-xs text-emerald-600 font-bold uppercase tracking-wide mt-1">${TF(t,'role')}</div>
                </div>
              </div>
            </div>`; 
  }
  ...
  function applyForm(pos){ 
    const inputClass = "w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none focus:border-emerald-500 focus:bg-white focus:ring-4 focus:ring-emerald-500/10 transition-all";
    return `<form name="apply" method="POST" data-netlify="true" netlify-honeypot="bot-field" enctype="multipart/form-data">
      ...
      <div class="space-y-4">
        <div><label class="block text-xs font-bold text-slate-700 mb-1.5 uppercase">Họ tên</label><input class="${inputClass}" type="text" name="name" required></div>
        <div class="grid grid-cols-2 gap-4">
          <div><label class="block text-xs font-bold text-slate-700 mb-1.5 uppercase">Email</label><input class="${inputClass}" type="email" name="email" required></div>
          <div><label class="block text-xs font-bold text-slate-700 mb-1.5 uppercase">SĐT</label><input class="${inputClass}" type="tel" name="phone" required></div>
        </div>
        <div><label class="block text-xs font-bold text-slate-700 mb-1.5 uppercase">CV</label><input class="${inputClass}" type="file" name="cv" accept=".pdf,.doc,.docx" required></div>
        <div><label class="block text-xs font-bold text-slate-700 mb-1.5 uppercase">Portfolio Link</label><input class="${inputClass}" type="url" name="resume"></div>
        <div><label class="block text-xs font-bold text-slate-700 mb-1.5 uppercase">Giới thiệu</label><textarea class="${inputClass}" name="message" rows="3" required></textarea></div>
        <button class="w-full rounded-xl bg-emerald-600 text-white font-bold text-sm px-6 py-3.5 hover:bg-emerald-700 transition-all shadow-lg mt-2" type="submit">Gửi hồ sơ</button>
      </div>
    </form>`; 
  }
  ...
  window.openApplyModal = function(pos){
    ...
    '<div class="relative z-10 max-w-md w-full bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]" style="animation: modalPop 0.3s ease-out forwards;">' +
      '<div class="p-5 border-b border-slate-100 flex items-center justify-between bg-white sticky top-0 z-20">' +
        '<div class="font-bold text-slate-800 text-lg">Ứng tuyển: ' + (pos||'Vị trí') + '</div>' +
        '<button type="button" class="h-8 w-8 inline-flex items-center justify-center rounded-full bg-slate-100 text-slate-500 hover:bg-red-50 hover:text-red-500 transition-colors" data-close-apply><i data-lucide="x" class="w-4 h-4"></i></button>' +
      '</div>' +
      '<div class="p-6 overflow-y-auto bg-white">' + applyForm(pos) + '</div>' +
    '</div>';
  ...
  function renderSection(s){ 
    ...
    else if(t==='text') { var parsed = window.marked ? marked.parse(lang()==='en'&&s.body_en?s.body_en:(s.body||'')) : (s.body||''); bodyContent = `<div class="${cardGlass()} p-8 shadow-lg" data-aos="fade-right"><div class="prose prose-slate prose-lg max-w-none text-slate-600">${parsed}</div></div>`; } 
  ...
  // [CHỈNH SỬA] LOGO ĐỐI TÁC: CÓ MÀU (Bỏ grayscale, tăng size)
  function partnerLogo(p){ 
    var src = (typeof p==='string')?p:(p&&(p.image||p.logo||p.url))||''; 
    // Logo hiện màu gốc (không có class grayscale)
    return `<div class="flex-shrink-0 mx-8 h-20 flex items-center justify-center hover:scale-110 transition-transform duration-300"><img src="${src}" class="h-full w-auto object-contain" loading="lazy"></div>`; 
  }
  ...
  function renderHome(el, home, svc, crs, port){
    ...
    if(home.hero && home.hero.fullscreen){
      ...
      // HERO SPLIT: Có container giới hạn (không tràn)
...
function renderGeneric(el, data, type){
    // [FIX] WrapperContainer: Đảm bảo nội dung các trang con nằm gọn trong khung max-w-6xl
    var wrapperStart = `<div class="max-w-6xl mx-auto px-4 py-8">`;
    var wrapperEnd = `</div>`;
    
    var head = h1(TF(data,'title'));
    // Cover: Vẫn dùng container để gọn, không tràn
    ...
    if(type==='services') list = `<div class="${gridThree()}">${(data.items||[]).map((it,i)=>serviceCard(it,i)).join('')}</div>`;
    if(type==='courses') list = `<div class="${gridThree()}">${(data.items||[]).map((it,i)=>courseCard(it,i)).join('')}</div>`;
    if(type==='portfolio') list = `<div class="${gridThree()}">${(data.items||[]).map((it,i)=>caseCard(it,i)).join('')}</div>`;

    // Render: Header + Cover + Nội dung trang + List items + Sections tùy chỉnh
  ...
  async function loadAndRenderContent(){
    ...
    // 2. NEWS DETAIL
        ...
        var head = h1(TF(item,'title'));
        var cover = item.image ? `<div data-aos="zoom-in" class="relative rounded-2xl overflow-hidden shadow-lg mb-8"><img class="w-full h-auto object-cover" src="${item.image}"></div>` : '';
        var meta = item.date ? `<div class="flex items-center gap-4 text-sm text-slate-500 mb-6 pb-4 border-b border-slate-100"><span>${item.date}</span></div>` : '';
        var body = `<article class="bg-white p-8 md:p-10 rounded-2xl border border-slate-100 shadow-sm" data-aos="fade-up"><div class="prose prose-slate prose-lg max-w-none text-slate-700">${window.marked?marked.parse(TF(item,'body')||''):TF(item,'body')}</div></article>`;
