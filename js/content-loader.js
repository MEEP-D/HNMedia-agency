(function(){
  ...
  // --- RENDER LOGO ĐỐI TÁC (Đã sửa lỗi hiển thị nhiều đối tác cuộn 1 hàng) ---
  function partnerLogo(p){ 
    var src = (typeof p==='string') ? p : (p && (p.image||p.logo||p.url)) || ''; 
    var alt = (p && (p.name||p.title)) || TR('Đối tác','Partner'); 
    return `<div class="flex-shrink-0 mx-6 h-16 flex items-center justify-center min-w-[150px]">
              <img src="${src}" alt="${alt}" class="h-full w-auto object-contain filter grayscale hover:grayscale-0 transition duration-300" loading="lazy" decoding="async">
            </div>`; 
  }
  ...
  function renderPartners(home){ 
    ...
    var marqueeHtml = `<section class="py-6 reveal">
                          ...
                          .marquee-track img {
                            height: 4rem; /* Chiều cao cố định cho logo */
                          }
                          </style>
                        </section>`;
  ...
  function renderHome(el, home, services, courses, portfolio){
    ...
    var mediaToUse = heroVideo || heroGif || heroImg;
    var mediaTag = heroVideo ? `<video src="${heroVideo}" autoplay loop muted playsinline class="w-full h-auto rounded-2xl shadow-xl object-cover"></video>` 
                             : `<img class="w-full h-auto rounded-2xl shadow-xl object-cover" src="${mediaToUse}" alt="${(home.hero && home.hero.title) || 'Hero'}" loading="eager" decoding="async">`;

    // Áp dụng layout_style (Đọc từ CMS)
    var layoutStyle = (home.hero && home.hero.layout_style) || 'content_left_media_right';

    var layoutClasses = 'flex-col md:flex-row';
    var mediaContainerClasses = 'w-full md:w-1/2';
    var contentContainerClasses = 'w-full md:w-1/2';

    // Logic bố cục cho Hero
    if (layoutStyle === 'media_full_width') {
        layoutClasses = 'flex-col';
        mediaContainerClasses = 'w-full order-first mb-6';
        contentContainerClasses = 'w-full';
    } else if (layoutStyle === 'media_center_small') {
        layoutClasses = 'flex-col items-center text-center';
        mediaContainerClasses = 'w-48 h-48 mx-auto mb-8';
        contentContainerClasses = 'w-full';
    } else { // content_left_media_right (mặc định)
        layoutClasses = 'flex-col md:flex-row';
        mediaContainerClasses = 'w-full md:w-1/2';
        contentContainerClasses = 'w-full md:w-1/2';
    }

    var heroContent = `
        <div class="relative z-10 max-w-6xl mx-auto px-4 py-12 md:py-20 flex ${layoutClasses} items-center gap-8 min-h-[50vh]">
            <div class="${contentContainerClasses} order-2 md:order-1 space-y-4">
                <p class="text-emerald-600 font-semibold text-sm">${home.hero && home.hero.slogan || ''}</p>
                <h1 class="text-4xl md:text-5xl font-bold leading-tight text-slate-900">${home.hero && home.hero.title || ''}</h1>
                <p class="text-slate-600 text-lg">${home.hero && home.hero.subtitle || ''}</p>
                ${home.hero && home.hero.ctaText ? `<a class="inline-flex items-center gap-2 rounded-xl bg-emerald-600 text-white px-6 py-3 font-semibold hover:bg-emerald-700 transition duration-300" href="contact.html">
                    <i data-lucide="arrow-right" class="h-5 w-5"></i>
                    <span>${home.hero.ctaText}</span>
                </a>` : ''}
            </div>
            ${useMedia ? `
            <div class="${mediaContainerClasses} order-1 md:order-2">
                ${mediaTag}
            </div>
            ` : ''}
        </div>
    ...
    var hero = `<section class="py-0 reveal">${heroContent}</section>`;
