(function(){
  ...
  const AOS_CONFIG = {
    ...
    offset: 60,           // Kích hoạt sớm 60px trước khi hiện
    duration: 800,        // Thời gian chạy 0.8s
  ...
  // --- 2. XỬ LÝ ĐIỀU HƯỚNG (FIX LỖI KHÔNG VỀ ĐƯỢC TRANG CHỦ) ---
  function setupNavigation() {
    document.body.addEventListener('click', async (e) => {
        ...
        if (link) {
            ...
            // Scroll lên đầu và tải lại nội dung
            window.scrollTo(0, 0);
  ...
  // --- 3. HÀM UI & STYLE (VISUAL) ---
  
  // Tiêu đề: Font text-xl, có gạch chân xanh hiện đại
  ...
  // Xử lý Background từ CMS (Màu, Ảnh, Gradient)
  function getBackgroundStyle(bg) {
    ...
    if (bg.type === 'gradient') return `background-image: ${bg.gradient};`;
    return '';
  }

  // Khung Section bao ngoài
  ...
  // Style Card cơ bản (Hiệu ứng Hover đẹp)
  ...
  function newsCard(n, i){ 
    ...
    return `<div ${cardBase(i)}>
              ...
              <div class="p-5 flex flex-col flex-1">
                ...
                <div class="text-slate-400 text-xs font-bold uppercase tracking-wider group-hover:text-emerald-600 transition-colors">${TR('Đọc tiếp','Read more')} &rarr;</div>
              </div>
            </div>`; 
  }

  // Card Ứng tuyển - Gọi Modal
  ...
  // --- 5. MODAL ỨNG TUYỂN (FIX LỖI HIỂN THỊ) ---
  ...
  // Dùng CSS animation thay AOS để tránh lỗi modal không hiện
  ...
  // Logo đối tác có màu (đã xóa class grayscale)
